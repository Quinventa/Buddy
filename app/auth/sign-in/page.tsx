...
function SubmitButton({ language }: { language: "en" | "nl" }) {
...
export default function SignInPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.log("[v0] OAuth signInWithOAuth result:", { data, error })

      if (error) {
        console.error("[v0] Google OAuth error:", error)
        setGoogleError(error.message || t("failedToSignInWithGoogle"))
        setGoogleLoading(false)
      }
      // If no error, user will be redirected to Google
    } catch (err) {
      console.error("[v0] Google OAuth error:", err)
      setGoogleError(t("googleSignInNotAvailable"))
      setGoogleLoading(false)
    }
  }
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {googleError && <div className="text-red-600 text-sm bg-red-50 p-3 rounded-md">{googleError}</div>}
